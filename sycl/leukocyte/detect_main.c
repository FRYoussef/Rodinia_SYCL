...
int main(int argc, char ** argv) {
	...
	buffer<float,1> d_gicov(host_gicov, grad_m * grad_n, props);
...
#endif
	auto host_gicov_acc = d_gicov.get_access<sycl_read>();
	MAT *gicov = m_get(grad_m, grad_n);
	for (int m = 0; m < grad_m; m++)
		for (int n = 0; n < grad_n; n++) {
#ifdef DEBUG
			printf("host_gicov: %f\n", host_gicov_acc[(n * grad_m) + m]);
#endif
			m_set_val(gicov, m, n, host_gicov_acc[(n * grad_m) + m]);
		}
...
#include "kernel_dilated.sycl"
	...
	auto host_img_dilated_acc = d_img_dilated.get_access<sycl_read>();
	...
	for (int m = 0; m < max_gicov_m; m++)
		for (int n = 0; n < max_gicov_n; n++) {
			m_set_val(img_dilated, m, n, host_img_dilated_acc[(m * max_gicov_n) + n]);
#ifdef DEBUG
			printf("host_img_dilated: %f\n", host_img_dilated_acc[(m * max_gicov_n) + n]);
#endif
		}
